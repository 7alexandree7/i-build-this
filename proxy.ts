...
export default clerkMiddleware(async (auth) => {
    ...
    if (userId && orgId) {
        try {
            ...
            const { data: organization } = await client.users.getOrganizationMembershipList({ userId: userId })
            ...
            await client.organizations.createOrganization({name: orgName, createdBy: userId});
        } catch (error) {
            console.error(error);
        }
    }
